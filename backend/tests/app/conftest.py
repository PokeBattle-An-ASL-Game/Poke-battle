...
FAKE_MANIFEST = ModelManifest("fake-v1", "fake", ("CITY", "TABLE", "NO"), frozenset({"CITY", "TABLE"}), Path("x"))


class FakeRecognizer:
    def __init__(self, prediction=None, error=None):
        self.prediction = prediction or Prediction("CITY", 0.9, None, "fake-v1")
        self.error = error
        self.calls = 0
...
def game_files(tmp_path):
    ...
    levels.mkdir()
    for level_id, available, signs in ((1, True, ["CITY", "TABLE"]), (2, False, ["CITY", "TABLE"]), (3, True, ["NO", "TABLE"])):
    ...
    registry = [{"id": s, "modelLabel": None if s == "NO" else s.lower()} for s in ("CITY", "TABLE", "NO")]
